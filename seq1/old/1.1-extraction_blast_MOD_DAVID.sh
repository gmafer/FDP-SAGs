...
#SBATCH --output=seq1/data/logs/extract_BLAST_%A_%a.out
#SBATCH --error=seq1/data/logs/extract_BLAST_%A_%a.err
...
FASTA_OUT_DIR=seq1/data/mtags
BLAST_OUT_DIR=seq1/data/blast
SAMPLE=$(ls ${DATA_DIR} | awk -F'.' '{print $1}' | grep GC | sort -u | awk "NR == ${SLURM_ARRAY_TASK_ID}")
